...
jQuery(function ($) {
    ...
    $('.c-button-section').on('mouseover', function (e) {
        debug && (console.log('..inspect before write'))
        requredls = typeof window.ShapeReqFields != 'undefined' && {};
        ...
        if (sO.length === checkall) {
            ...
            for (var key in requredls) {
                !i.isEmpty('c-text-' + requredls[key]) && !$("." + key).find(".c-editor > input").val() && n.invalidStyle($("." + key))
            }
        }
    });
    ...
    ofr.on('submit', function (e) {
        ...
        $.post(ajaxPath, ajaxArr, function (answer) {
            ...
            if (typeof answer['required'] != 'undefined') {
                for (it in answer['required']) {
                    console.log($(".c-text-" + answer['required'][it]).find(".c-editor > input").val());
                    n.invalidStyle($(".c-text-" + answer['required'][it]));
                }
            }
